...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Target 95%% half-width for run-to-run MW drift across repeated iterations.",
    )
    ...
    *,
    ...
    for iteration in range(1, args.max_iterations + 1):
        ...
        iteration_frames.append(
            read_mw_summary(run_dir, stage="iteration_pilot", iteration=iteration)
        )
    ...
    if not target_met and chosen_iterations >= args.min_iterations:
        chosen_iterations = choose_mw_iterations(
            iteration_summary,
            target_half_width=args.target_iteration_half_width,
...
def main() -> None:
    ...
    recommendation = {
        ...
        "methodology": {
            "shot_rule": (
                "Choose smallest shot count whose max consecutive MW mean change is <= shot_tolerance."
            ),
            "sample_rule": (
                "Choose smallest n_samples whose worst 95% half-width is <= target_half_width; "
                "otherwise use required_n_samples."
            ),
            "iteration_rule": (
