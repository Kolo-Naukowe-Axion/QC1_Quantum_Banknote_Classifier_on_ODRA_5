...
\begin{abstract}
In quantum machine learning the ansatz is a model's trainable architecture, and candidate ans\"atze are ranked before training by pre-training metrics, such as expressibility, Meyer--Wallach (MW) entanglement, and state fidelity.
These are almost always computed on noiseless simulators.
Whether such simulator rankings survive on a physical QPU is untested, yet that is exactly the regime in which they are used.
We introduce a pilot-calibrated on-device verification protocol built on three hardware-native measures: MW entanglement from reduced-state tomography, output-state fidelity from full Pauli-basis tomography, and KL expressibility of QPU pairwise fidelities against the Haar reference -- each with a pilot stage that bounds the QPU shots, samples, and repeats needed to resolve it.
Applied on the IQM Spark ODRA five-qubit processor to a hardware-aligned ring ansatz versus a controlled-rotation baseline, we find a systematic simulator--hardware mismatch.
On hardware only tomographic fidelity still separates the ans\"atze, and task accuracy confirms it: at depth~4 the native-aligned circuit keeps $87.6\%$ ($F_1 = 0.85$) versus $74.5\%$ ($F_1 = 0.58$), where simulation ties them.
MW and KL saturate under noise and lose discriminative power; at depth~6 both ans\"atze collapse.
For this device and these families, simulator-only rankings mislead architecture choice.
Low-cost on-device verification should precede committing hardware time to training, a step toward hardware-aware QML on NISQ devices.
\end{abstract}
...
Parameterized quantum circuits are used across QML, simulation, and optimization; on near-term hardware their behaviour depends jointly on ansatz structure, data encoding, and transpilation to native gates -- not on ansatz choice alone~\cite{sim2019,huang2024}.
To avoid costly full training, practitioners rank candidate ans\"atze with pre-training metrics such as Meyer--Wallach (MW) entanglement, KL-divergence-based expressibility, and state fidelity from quantum state tomography, which are almost always computed on ideal, noiseless simulators.
Such metrics are proxies for expressivity and trainability, the same properties that govern optimizability and generalization in a QML model, which is why their ranking is trusted to guide architecture choice.
For two fixed ans\"atze matched in depth and parameter count, it is unknown whether a simulator ranking survives on a physical QPU, where limited connectivity, gate decomposition, and calibration drift can penalize architectures unevenly.

We make two contributions to bridge this gap between simulation-based QML design and physical deployment.
First, we present a reproducible on-device verification protocol built on three hardware-native measures: MW entanglement from single-qubit reduced-state tomography, output-state fidelity from full Pauli-basis tomography, and KL expressibility of QPU pairwise fidelities against the Haar reference, each preceded by a pilot stage that bounds the required QPU compute by selecting the smallest shot, sample, and repeat budget that resolves the metric against measurement noise and calibration drift.
Second, we give an experimental demonstration that ansatz selection driven by simulator evaluation does not lead to good choices for hardware deployment: measured on the device, the same metrics do not preserve the simulator ranking.
We instantiate this on the IQM Spark ODRA five-qubit processor (star topology; native $\mathrm{CZ}$ and $R(\theta,\phi)$ gates), comparing a hardware-aligned ring ansatz (\texttt{ansatz\_odra}) with a controlled-rotation, simulator-oriented baseline (\texttt{ansatz\_simulator}) on the four raw features (UCI Banknote Authentication task) plus one engineered interaction feature, angle-encoded on five qubits. A fixed angle encoding isolates ansatz and transpilation effects, and end-to-end classification on the same checkpoints provides an independent, task-level hardware axis.

Our central result is a systematic simulator--hardware mismatch that undermines naive QML model selection.
In simulation MW separates the two ans\"atze at shallow depth (depth~2: $0.52$ vs.\ $0.44$); on hardware the gap vanishes as noise-induced mixing inflates MW on mixed reduced states toward uniform-randomness levels ($0.69$ vs.\ $0.70$, overlapping 95\% confidence intervals).
Simulator KL stays near the Haar reference, whereas on the QPU the pairwise-fidelity distribution diverges from both Haar and simulation ($D_{\mathrm{KL}} \approx 11$) while mean overlaps remain small ($|\langle\psi_a|\psi_b\rangle|^2 \approx 0.03$)---consistent with noisy, nearly orthogonal mixed states rather than mode collapse.
Of the three measures, only tomographic fidelity still resolves the two ans\"atze on hardware, localizing the degradation to the transpilation-inflated two-qubit-gate overhead of the non-native baseline.
Classification mirrors this at depth~4: simulation ties the ans\"atze (88.0\% vs.\ 88.2\%), but on the QPU the native-aligned circuit retains 87.6\% accuracy ($F_1 = 0.846$) versus 74.5\% ($F_1 = 0.583$) for the baseline; at depth~6 both collapse once the noise budget is exceeded.

We release pilot scripts, calibrated protocols, and analysis artifacts as open source.
Scope is intentionally narrow---one QPU, one dataset, five qubits, two ansatz families---matching the scale of an empirical protocol paper.
Simulator-only metric rankings should not stand alone; low-cost on-device verification ought to complement simulated benchmarking before committing hardware time to training.
...
For each test input $x$, we build the inference circuit by combining an angle-encoding feature map, the ansatz and random weights $\theta^{*}$, $\theta^{*} \in [0,2\pi]$. We then define two states for the same $(x,\theta^{*})$ pair: (i) the ideal reference $|\psi_{\mathrm{ideal}}\rangle$, obtained by noiseless
statevector simulation of this circuit, and
(ii) the hardware state $\rho_{\mathrm{hardware}}$, reconstructed from QPU measurement
data after executing the same circuit on IQM Spark.
We quantify state-preparation quality using \emph{physical-projection fidelity}
\begin{equation}
  \mathcal{F}_{\mathrm{phys}}
  =
  \langle \psi_{\mathrm{ideal}} \mid \rho_{\mathrm{hardware}} \mid \psi_{\mathrm{ideal}} \rangle,
  \label{eq:fidelity}
\end{equation}
where $\mathcal{F}_{\mathrm{phys}} \in [0,1]$ and higher values indicate better agreement
between hardware and ideal state preparation.
We report $\mathcal{F}_{\mathrm{phys}}$ rather than fidelity from the raw reconstructed
matrix, because finite-shot tomography can produce unphysical estimates.

\paragraph{Tomography protocol.}
For $n=5$ qubits, we perform full tensor-product Pauli tomography over all
$3^n=243$ measurement settings $b \in \{X,Y,Z\}^{\otimes n}$.
For each setting, we append the corresponding basis rotations to the bound circuit,
measure in the computational basis with $N_s$ shots, and estimate Pauli expectations from
parity-weighted counts:
\begin{equation}
  \langle \widehat{P}\rangle
  \;=\;
  \frac{1}{N_s}\sum_{s=1}^{N_s}\chi_P(s),
  \qquad \chi_P(s)\in\{-1,+1\}.
  \label{eq:pauli-estimator}
\end{equation}
The density matrix is reconstructed by linear inversion,
\begin{equation}
  \hat{\rho}
  \;=\;
  \frac{1}{2^n}\sum_{P\in\{I,X,Y,Z\}^{\otimes n}}
  \langle \widehat{P}\rangle\, P,
  \label{eq:linear-inversion}
\end{equation}
and projected onto the physical set:
$\rho_{\mathrm{phys}}=\mathrm{Proj}_{\mathrm{physical}}(\hat{\rho})$.
The reported hardware state is $\rho_{\mathrm{hardware}}=\rho_{\mathrm{phys}}$, and fidelity
is computed using Eq.~\eqref{eq:fidelity}.
Tomography circuits are transpiled to the native IQM Spark gate set and submitted in batches;
each prepared state therefore requires $243$ measurement circuits, each executed with
$N_s$ shots (pilot-calibrated; we use $N_s=1024$).


\subsection{Meyer--Wallach entanglement}

\paragraph{Metric definition.}
We quantify each ansatz's entangling capability~\cite{sim2019} with the
Meyer--Wallach (MW) global entanglement measure~\cite{meyerwallach2002}. For an
$n$-qubit pure state $|\psi\rangle$,
\begin{equation}
  Q(|\psi\rangle)
  =
  \frac{2}{n}\sum_{i=1}^{n}\bigl(1-\operatorname{Tr}\rho_i^{2}\bigr),
  \label{eq:mw-def}
\end{equation}
where $\rho_i$ is the single-qubit reduced density matrix of qubit $i$;
$Q=0$ for a product state and $Q=1$ for a maximally entangled state.
Brennen~\cite{brennen2003} showed that \eqref{eq:mw-def} equals the average
single-qubit linear entropy, so $Q$ is fixed by the one-qubit marginals alone
and needs no full $2^{n}\times 2^{n}$ reconstruction. Writing each marginal in
Bloch form $\rho_i=\tfrac12(I+x_iX+y_iY+z_iZ)$ with $x_i=\langle X_i\rangle$,
$y_i=\langle Y_i\rangle$, $z_i=\langle Z_i\rangle$, its purity is
$\operatorname{Tr}\rho_i^{2}=\tfrac12(1+x_i^{2}+y_i^{2}+z_i^{2})$
and \eqref{eq:mw-def} reduces to a form built only from Pauli expectations,
\begin{equation}
  Q = 1-\frac{1}{n}\sum_{i=1}^{n}\bigl(x_i^{2}+y_i^{2}+z_i^{2}\bigr).
  \label{eq:mw-bloch}
\end{equation}

\paragraph{Estimator and protocol.}
Equation~\eqref{eq:mw-bloch} is the on-device estimator: for each sampled state,
three measurement circuits ($Z$, $X$, $Y$ basis) yield the five Bloch vectors and
$\hat Q$ follows directly, so one MW sample costs only three circuits rather than
$3^{n}$. Trainable parameters are drawn uniformly on $[0,2\pi)^{p}$; for a fixed
(ansatz, depth) we report the parameter-averaged entanglement
$\bar Q=\tfrac1N\sum_{j=1}^{N}\hat Q_j$ over $N$ random draws, with the sample
spread $s_Q$ (variation across parameters) distinct from the error bar on the
mean $s_Q/\sqrt N$. The idealized reference is the exact statevector value of
$Q$; the hardware estimate uses finite-shot Pauli histograms, whose delta-method
shot-noise bound is $\sigma_{\mathrm{shot}}\lesssim 2/\sqrt{nS}$ per sample and
$\lesssim 2/\sqrt{nSN}$ after averaging ($\approx 3.6\times10^{-3}$ MW units for
$n{=}5$, $S{=}1024$, $N{=}60$).

\paragraph{Pilot-calibrated budget.}
The hardware budget $(S,N,K)$ is fixed by a three-stage pilot before the
production run. Sweeping $S\in\{512,1024,2048,4096\}$, the largest consecutive
change in $\bar Q$ falls to $\le0.02$ at $S=1024$, the smallest count meeting the
stability tolerance. Sizing $N$ to a $95\%$ half-width target of $0.03$ MW units,
the worst-case (shallow, high-variance depth-2) configuration needs $N=59$; we
adopt $N=60$. Repeating the frozen protocol $K=3$ times under an identical seed
isolates QPU drift from parameter sampling; the worst-case across-iteration
$95\%$ half-width is $6.1\times10^{-3}$ MW units, inside the $0.01$ target.


\subsection{KL expressibility (simulator and QPU vs.\ Haar)}

\paragraph{Metric definition.}
Following Sim et~al.~\cite{sim2019}, we measure expressibility by how closely the
ansatz-induced distribution of pairwise state fidelities matches the Haar
reference.
For an $n$-qubit circuit ($n=5$, Hilbert-space dimension $d=2^n$) we draw
independent parameter pairs $(\theta_a,\theta_b)$ uniformly on $[0,2\pi)^{p}$,
prepare the corresponding states, and record the fidelity
$F=|\langle\psi(\theta_a)|\psi(\theta_b)\rangle|^2$ (simulator) or its
tomographic analogue on hardware (below).
The Haar law on fidelities is
\begin{equation}
  P_{\mathrm{Haar}}(F)
  =
  (d-1)\,(1-F)^{d-2},
  \qquad F\in[0,1].
  \label{eq:haar-fidelity}
\end{equation}
We form empirical histograms $\hat P$ on a shared uniform grid of $B$ bins over
$[0,1]$ and report the Kullback--Leibler divergence
\begin{equation}
  D_{\mathrm{KL}}\!\bigl(\hat P\,\big\|\,P_{\mathrm{Haar}}\bigr)
  =
  \sum_{b=1}^{B}
  \hat P_b\,
  \log\!\frac{\hat P_b+\varepsilon}{P_{\mathrm{Haar},b}+\varepsilon},
  \label{eq:kl-expressibility}
\end{equation}
with additive smoothing $\varepsilon=10^{-12}$ and renormalization of both
discrete measures.
Lower $D_{\mathrm{KL}}$ means closer agreement with Haar (higher expressibility).
Circuits are bare ans\"atze: no feature map or dataset input is included, so the
metric isolates the trainable unitary family.
We evaluate three families at depths $L\in\{2,4,6\}$: the hardware-aligned ring
(\texttt{ansatz\_odra}), the controlled-rotation baseline
(\texttt{ansatz\_simulator}), and the star-topology ansatz
(\texttt{ansatz\_star}).

\paragraph{Simulator protocol (expressibility as in Sim et~al.).}
On the noiseless statevector backend we estimate each pairwise fidelity exactly
as $|\langle\psi_a|\psi_b\rangle|^2$.
Per (ansatz, depth) we draw $N_{\mathrm{sim}}=5000$ independent parameter pairs
and bin with $B=75$ bins, matching the classical expressibility setup of
Sim et~al.~\cite{sim2019}.
The headline simulator score is
$D_{\mathrm{KL}}(\hat P_{\mathrm{sim}}\|P_{\mathrm{Haar}})$.

\paragraph{Hardware pairwise fidelity.}
On IQM Spark ODRA~5 the same parameter pairs cannot be compared via pure-state
inner products.
For each pair we reconstruct density matrices $\rho_a$ and $\rho_b$ by full
$3^n=243$-setting Pauli tomography (identical linear-inversion and physical
projection protocol as in the fidelity subsection), using $S$ shots per setting,
and take the hardware overlap
\begin{equation}
  F_{\mathrm{hw}}
  =
  \operatorname{Tr}(\rho_a\rho_b)
  \label{eq:hw-pairwise-fidelity}
\end{equation}
computed on the physically projected estimates.
Each fidelity sample therefore costs $2\times 243$ tomography circuits.
Circuits are transpiled to the native ODRA gate set before submission.

\paragraph{QPU budget and uncertainty (fixed sample protocol).}
Unlike MW, the KL hardware budget is fixed rather than pilot-adapted.
For each (ansatz, depth) we collect $N_{\mathrm{pair}}=60$ fidelity pairs per
iteration over $K=2$ independent QPU iterations (identical parameter seed,
separate calibrations), yielding $N_{\mathrm{QPU}}=120$ pooled hardware
fidelities.
We use $S=2048$ shots per tomography setting and the same $B=75$ bins as the
simulator so that $D_{\mathrm{KL}}$ values are directly comparable.
The two headline scores are the classical expressibility metric
$D_{\mathrm{KL}}(\hat P_{\mathrm{sim}}\|P_{\mathrm{Haar}})$ and its on-device
counterpart $D_{\mathrm{KL}}(\hat P_{\mathrm{QPU}}\|P_{\mathrm{Haar}})$.
Uncertainty on the QPU estimate is reported via percentile bootstrap:
$N_{\mathrm{boot}}=5000$ resamples of the pooled fidelities, with $90\%$ and
...
\paragraph{Meyer--Wallach entanglement.}
Table~\ref{tab:mw} and Figure~\ref{fig:mw} report $\bar Q$ for both ans\"atze at
depths $L\in\{2,4,6\}$, first on the exact statevector reference ($N=500$) and
then measured on ODRA~5 ($N=60$, $S=1024$, averaged over $K=3$ fixed-seed
iterations). In simulation the hardware-aligned ODRA ansatz is the more
entangling family at every depth, and the separation is decisive: $0.516$ vs.\
$0.441$ at depth~2 (a $0.075$ gap at $6.6\sigma$ in units of the combined
standard error), $7.7\sigma$ at depth~4, and $5.3\sigma$ at depth~6. On the
strength of the simulator alone one would rank ODRA above the baseline and treat
shallow depth as the regime where the entangler choice matters most.

Measured on hardware, that ranking does not survive. At depth~2 the two families
are statistically indistinguishable---$\bar Q=0.693$ (ODRA) vs.\ $0.699$
(baseline), a $-0.006$ difference at $0.3\sigma$ with the sign reversed relative
to simulation---and depth~4 behaves identically ($0.941$ vs.\ $0.940$,
$0.2\sigma$). The mechanism is noise-induced state mixing: gate and readout
errors drive each single-qubit marginal toward the maximally mixed state, which
lowers every $\operatorname{Tr}\rho_i^{2}$ and therefore \emph{inflates} $Q$
toward its ceiling. Both shallow-depth hardware scores sit near $0.69$---far
above their simulator values and close to the level of nearly-mixed reduced
states---so the inflation swamps the genuine $0.075$ entanglement difference
exactly where the simulator claimed the strongest signal. The residual
$4.3\sigma$ separation at depth~6 ($0.975$ vs.\ $0.967$) is not recovered
discriminating power: both scores have saturated within $0.03$ of $Q=1$, where
the tiny error bars (SEM $\approx0.001$) let a physically inconsequential
difference clear the significance bar. MW near unity at depth~6 is a signature of
decoherence, the same noise-budget-exceeded regime in which the end-to-end
classifier collapses. MW thus ranks the two ans\"atze crisply in simulation and
provides no usable ranking on the device.
...
\begin{table}[t]
  ...
  \begin{tabular}{c cc c cc c}
    ...
    \textbf{$L$} & ODRA & Base. & Sep. & ODRA & Base. & Sep. \\
    \midrule
    2 & $0.516$ & $0.441$ & $6.6\sigma$ & $0.693$ & $0.699$ & $0.3\sigma$ \\
    4 & $0.807$ & $0.752$ & $7.7\sigma$ & $0.941$ & $0.940$ & $0.2\sigma$ \\
    6 & $0.878$ & $0.855$ & $5.3\sigma$ & $0.975$ & $0.967$ & $4.3\sigma$ \\
    \bottomrule
  \end{tabular}
  \caption{Meyer--Wallach entanglement $\bar Q$ for the hardware-aligned (ODRA)
  and controlled-rotation baseline ans\"atze. ``Sep.'' is the between-ansatz
  difference in units of its combined standard error. Simulator: exact
  statevector, $N=500$. Hardware: ODRA~5, $N=60$, $S=1024$, $K=3$. The $5$--$8\sigma$
  simulator separations collapse to $\lesssim0.3\sigma$ at depths~2 and~4; the
  depth-6 residual is a saturation artifact (both scores near the $Q=1$
  noise-mixing ceiling), not a recovered signal.}
  \label{tab:mw}
\end{table}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{Figures/mw_sim_vs_hw}
  \caption{Mean Meyer--Wallach score $\bar Q$ by circuit depth, simulator (left)
  vs.\ ODRA~5 hardware (right); error bars are $\pm$SEM. In simulation the
  hardware-aligned ansatz sits a resolved $0.075$ above the baseline at depth~2;
  on hardware the two are indistinguishable ($-0.006$, $0.3\sigma$) as
  noise-induced mixing lifts both scores toward the maximally-mixed ceiling.}
  \label{fig:mw}
\end{figure}

\paragraph{KL expressibility (simulator and QPU vs.\ Haar).}
% TODO: Table~\ref{tab:kl} --- only two metrics:
%   (1) D_KL(P_sim || P_Haar)  [expressibility]
%   (2) D_KL(P_QPU || P_Haar)
% for ansatz_odra, ansatz_simulator, ansatz_star at L in {2,4,6};
% include bootstrap 90%/95% CIs for QPU (N_QPU=120, N_boot=5000).
% TODO: Figure~\ref{fig:kl} --- D_KL vs.\ depth (sim vs.\ QPU) and/or fidelity
% histograms vs.\ Haar density.
% TODO: Narrative: whether simulator expressibility rankings survive on QPU;
% whether D_KL(QPU||Haar) saturates / loses discriminative power under noise.
% Numbers intentionally omitted until final analysis CSV is locked.
