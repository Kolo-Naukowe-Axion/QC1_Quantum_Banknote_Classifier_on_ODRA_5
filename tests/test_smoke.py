...
def test_imports() -> None:
    ...
    assert (root / "star.py").is_file()


def test_star_ansatz_registry() -> None:
    registry = star_ansatz_registry(root=ROOT)
    assert list(registry) == ["ansatz_star"]
    for depth in (2, 4, 6):
        circuit = registry["ansatz_star"](5, depth)
        assert len(circuit.parameters) == star_param_count(5, depth)
...
def test_cli_argument_parsing() -> None:
    ...
    assert budget["shot_pairs"] == 45
    assert budget["total_pairs"] == 405
